...
load('SegmentationDataMatrices\p_x_mean.mat')
load('SegmentationDataMatrices\p_y_mean.mat')
...
for i = 1:n
    ...
    %Extrinsic Monte Carlo Values
    if ext
        H_c12c2_mc = H_c12c2_mu*expSE( wedgeSE( v_c12c2_mc(i,:) ) ); % Put randomly selected values back into H_c12c2 form
    else
        H_c12c2_mc = H_c12c2_MoCap;
    end
    %% Point Recovery
    %Depth Recovery
    [zc1, zc2] = recoverDepth(p_m1_mc{i}, p_m2_mc{i}, A_c2m_mc{1,i}, A_c2m_mc{2,i}, invSE(H_c12c2_mc));% Recover depth using random transform above
...
for k=1:2
    trueball(k) = plot(h_d2a(k),0, 0, 'g*','MarkerSize',10,'LineWidth',1);
